...
class TestFlow(Flow):
    ...
    def run(self, deposit_id, user_id, version_id, bucket_id, key,
        ...
        with mock.patch(
                "cds.modules.flows.serializers.FlowResponseSerializer.build_flow_status_json",
                staticmethod(mock_build_flow_status_json)):
            with mock.patch(
                    "cds.modules.flows.models.Status.compute_status",
                    classmethod(mock_compute_status)):
                FlowResponseSerializer.update(flow=flow,
                                              version_id=version_id)
                FlowResponseSerializer.update_tasks(flow)
                FlowResponseSerializer.update_global_status(flow=flow)
                return flow
